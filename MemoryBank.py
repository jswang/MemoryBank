...
class MemoryBank:
    def __init__(self, nli_model, n_semantic, tokenizer, threshold=0.9):
        self.mem_bank = []
    ...
    def retrieve_from_index(self, s_new):
        ...
        lims, D, I = self.index.range_search(x=s_new, thresh=threshold)
        # I is the indices
        """
        ...
        for i in range(I.shape[-1]):
            retrieved.append(self.mem_bank(I[0, i]))
        return retrieved
...
def tester():
    ...
    qa_2 = ("Does an owl have a vertebrate?", "yes")
    mem_bank.add_to_bank(qa_1)
    s2 = mem_bank.encode_sents(" ".join(qa_2))
