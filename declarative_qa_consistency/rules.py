        ...
        if past:
            for tok in [root] + conj:
                if tok['form'] == 'leave':
                    ...
                    new_form = conjugator.conjugate(tok['form']).conjug_info['indicative']['indicative past tense']['1s']
                tok['form'] = new_form
                tok['xpostag'] = 'VBD'
        elif pres_3sg:
            for tok in [root] + conj:
                # tok['form'] = pattern.en.conjugate(tok['form'], tense='present', person=3, number='singular')
                tok['form'] = conjugator.conjugate(tok['form']).conjug_info['indicative']['indicative present']['3s']
                tok['xpostag'] = 'VBZ'
        return
